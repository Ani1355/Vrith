...
export const PricingCard = ({
  ...
  const cardClasses = `
    backdrop-blur-[14px] bg-gradient-to-br rounded-2xl shadow-xl w-full max-w-md px-10 py-12 flex flex-col transition-all duration-300
    from-white/80 to-white/60 border border-gray-200/50
    ${isPopular ? 'relative ring-2 ring-accent/20 shadow-2xl' : ''}
  `;
  // Use unified button styling
  const unifiedButtonVariant = buttonVariant === 'primary' ? 'primary' : 'secondary';

  return (
    <div className={cardClasses.trim()}>
      {isPopular && (
        <div className="absolute -top-4 right-4 px-3 py-1 text-[12px] font-semibold rounded-full bg-accent text-black">
          Most Popular
        </div>
      )}
      <div className="mb-3">
        <h2 className="text-[48px] font-extralight tracking-[-0.03em] text-[#1a1a1a] font-display">{planName}</h2>
        <p className="text-[16px] text-gray-700 mt-1 font-sans">{description}</p>
      </div>
      ...
      <ul className="flex flex-col gap-2 text-[14px] text-gray-700 mb-6 font-sans">
        {features.map((feature, index) => (
          <li key={index} className="flex items-center gap-2">
            <CheckIcon className="text-success w-4 h-4" /> {feature}
          </li>
        ))}
      </ul>
      <Button variant={unifiedButtonVariant} size="lg" className="mt-auto w-full">
        {buttonText}
      </Button>
    </div>
  );
...
export const ModernPricingPage = ({
  ...
  return (
    <div className="bg-[#f8f8f6] text-[#1a1a1a] w-full overflow-x-hidden">
      {showAnimatedBackground && <ShaderCanvas />}
      <main className="relative w-full flex flex-col items-center justify-center px-4 py-20">
        <div className="w-full max-w-5xl mx-auto text-center mb-14">
          <h1 className="text-[48px] md:text-[64px] font-extralight leading-tight tracking-[-0.03em] text-[#1a1a1a] font-display">
            {title}
          </h1>
          <p className="mt-3 text-[16px] md:text-[20px] text-gray-700 max-w-2xl mx-auto font-sans">
            {subtitle}
          </p>
        </div>
